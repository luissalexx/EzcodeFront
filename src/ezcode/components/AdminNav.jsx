...
export const AdminNav = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const response = await ezcodeApi.get(`admin/notificaciones/${userId}`);
    ...
    const handleLimpiar = async () => {
        try {
            const response = await ezcodeApi.post(`admin/notificaciones/${userId}`);
            if (response) {
            ...
            <AppBar position="static">
                <Toolbar>
                    ...
                    <Typography color='white' sx={{ flexGrow: 1 }}>
                        <Link variant='h6' component={RouterLink} to="/admin/" sx={{ textDecoration: 'none', color: 'inherit' }}>
                            Perfil
                        </Link>
                    </Typography>

                    <Typography color='white' sx={{ flexGrow: 1 }}>
                        <Link variant='h6' component={RouterLink} to="/admin/solicitudes" sx={{ textDecoration: 'none', color: 'inherit' }}>
                            Solicitudes
                        </Link>
                    </Typography>
